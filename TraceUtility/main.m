...
static int handleTrace(NSString *tracePath){
    NSError *error = nil;
...
//                        [contexts addObject:XRContextFromDetailNode(detailController, detailNode)];
            ...
            if ([instrumentID isEqualToString:@"com.apple.xray.instrument-type.coresampler2"]) {
                ...
                CaAllocations *allocations = [CaAllocations new];
                [allocations handle:instrument tracePath:tracePath];
...
int main(int argc, const char * argv[]) {
    @autoreleasepool {
        ...
        BOOL isdir = NO;
        [[NSFileManager defaultManager] fileExistsAtPath:tracePath isDirectory:&isdir];
        if (isdir) {
            NSArray<NSString *> *conts = [[NSFileManager defaultManager] contentsOfDirectoryAtPath:tracePath error:nil];
            for (NSString *path in conts) {
                if ([[[NSURL URLWithString:path] pathExtension] isEqualToString:@"trace"]) {
                    handleTrace([[[NSURL URLWithString:tracePath] URLByAppendingPathComponent:path] absoluteString]);
                }
            }
        } else {
            handleTrace(tracePath);
        }
        PFTClosePlugins();
    }
    return 0;
